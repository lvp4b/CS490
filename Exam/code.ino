...
Sensor* sensors[] = { &mhq, &pressure, &uv, &light, &dhtHumidity, &dhtTemperature };
...
class ThingSpeak : public Output {
public:
  ThingSpeak() : rest("api.thingspeak.com", 443) {}

  void begin() {
    rest.begin("<wifi ssid>", "<wifi password>");
  }

protected:
  void upload() {
    String url = "/update?api_key=<api key>";
    for (int i = 0; i != sizeof(sensors) / sizeof(sensors[0]); i++){
      url += "&field";
      url += i + 1;
      url += "=";
      url += sensors[i]->getValue();
    }
...
class AdaFruit : public Output {
public:
  AdaFruit() : rest("io.adafruit.com", 443) {}

protected:
  void upload() {
    // AdaFruit API is rate limited - use round robin for uploading data
    index++;
    ...
    String url = "/api/v2/lvp4b/feeds/";
    url += sensor->getFeedKey();
    url += "/data";

    String body = "value=";
    body += sensor->getValue();
    
    rest.setHeader("X-AIO-Key: <aio key>");
    rest.setContentType("application/x-www-form-urlencoded");
    int statusCode = rest.post(url.c_str(), body.c_str());
...
protected:
  void upload() {
    ...
    for (auto sensor : sensors) {
      ...
      buffer += ",";
      buffer += sensor->getLedState();
      buffer += ";";
    }

    bluetooth.write(buffer.length() - 1);
    bluetooth.write((byte*)buffer.c_str(), buffer.length() - 1);
    bluetooth.flush();
  }
  
  int getDelay() { return 250; }
...
Output* outputs[] = { &lcdOutput, &thingSpeak, &adaFruit, &mit };
